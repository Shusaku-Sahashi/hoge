import { mount } from '@vue/test-utils'
import KbnButton from '@/components/atoms/KbnButton.vue'

describe('KbnButton', () => {
  describe('プロパティー', () => {
    describe('type', () => {
      describe('デフォルト', () => {
        it('kbn-buttonクラスを持つbutton要素で構成されること', () => {
          const button = mount(KbnButton)
          expect(button.is('button')).to.equal(true)
          expect(button.classes('kbn-button')).to.include(true)
        })
      })
      describe('button', () => {
        it('kbn-buttonクラスを持つbutton要素で構成されていること', () => {
          const button = mount(KbnButton)
          button.setProps({ type: 'button' })
          expect(button.is('button')).to.equal(true)
          expect(button.classes()).to.include('kbn-button')
        })
      })
      describe('text', () => {
        it('kbn-button-textクラスを持つtext要素で構成されること', () => {
          const button = mount(KbnButton)
          button.setProps({
            type: 'text'
          })
          expect(button.is('button')).to.equal(true)
          expect(button.classes()).to.include('kbn-button-text')
        })
      })
    })
    describe('disabled', () => {
      describe('デフォルト', () => {
        it('disable属性が付与されていないこと', () => {
          const button = mount(KbnButton)
          expect(button.attributes().disabled).to.an('undefined')
        })
      })
      describe('true', () => {
        it('disabled属性が付与されていること', () => {
          const button = mount(KbnButton)
          button.setProps({
            disabled: true
          })
          expect(button.attributes().disabled).to.an('disabled')
        })
      })
      describe('false', () => {
        it('disabled属性が付与ていないこと', () => {
          const button = mount(KbnButton)
          button.setProps({
            disabled: false
          })
          expect(button.attributes().disabled).to.an('undefined')
        })
      })
    })
    describe('イベント', () => {
      describe('click', () => {
        it('発行されること', () => {
          const button = mount(KbnButton)
          button.trigger('click')
          expect(button.emitted().click.length).to.equal(1)
        })
      })
    })
    describe('slot', () => {
      describe('挿入あり', () => {
        it('挿入されていること', () => {
          const button = mount(KbnButton, {
            slots: {
              default: '<p>hello</p>'
            }
          })
          expect(button.text).is.equal('hello')
        })
      })
      describe('挿入なし', () => {
        it('挿入されていないこと', () => {
          const button = mount(KbnButton)
          expect(button.text).is.equal('')
        })
      })
    })
  })
})
